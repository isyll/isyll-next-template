-- Operator (administrator) sessions, isolated in the admin schema.
--
-- Mirrors app.sessions: in production operator sessions live exclusively in
-- Redis (BetterAuth `secondaryStorage`, short TTL). When `REDIS_URL` is unset,
-- BetterAuth falls back to its primary Drizzle adapter and persists operator
-- sessions here. The table MUST exist for that fallback to work.
--
-- `token` is an opaque admin session token. Timestamps are UTC.
